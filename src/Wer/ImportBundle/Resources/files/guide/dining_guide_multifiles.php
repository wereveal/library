...
$doc_root = "/srv/websites/www.rcreader.com/html/tools/test/guide_files";
$o_sec_file = fopen($doc_root . "/section.php", "w+");
...
if ($o_cats !== false) {
    ...
    $o_cats_file = fopen($doc_root . "/categories.php", "w+");
    $open_cats_file =
<<< EOT
<?php
\$a_cats = array(
EOT;
    fwrite($o_cats_file, $open_cats_file);
    foreach ($a_cats as $a_cat) {
        $o_item_file_list_file = fopen($doc_root . "/item_file_list_for_cat_{$a_cat['id']}.php");
...
EOT;
        ...
        if ($o_items !== false) {
            ...
            foreach ($a_items as $a_item) {
                $file_name = "item_{$a_item['itemid']}.php";
                fwrite($o_item_file_list_file, $file_name . ', ');
