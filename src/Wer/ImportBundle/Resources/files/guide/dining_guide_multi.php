...
if ($o_sections !== false) {
    $a_sections = $o_sections->fetch_all(MYSQLI_ASSOC);
    // print_r($a_sections);
    $o_section_files = fopen($doc_root . "section_files.php", "w+");
    $write_this =
<<<EOT
<?php
\$a_section_file_list = array(

EOT;
    fwrite($o_section_files, $write_this);
    foreach($a_sections as $a_section) {
        $sec_id = $a_section['catid'];
        $sec_file_name = "section_{$sec_id}.php";
        fwrite($o_section_files, "    '{$sec_file_name}',\n");
        $o_sec_file = fopen($doc_root . $sec_file_name, "w+");
        $write_this =
<<<EOT
<?php
\$a_sec_{$sec_id}_data = array(
    "sec_name"        => "{$a_section['name']}",
    "sec_description" => "{$a_section['introtext']} {$a_section['description']}",
    "sec_image"       => "",
    "sec_order"       => 1,
    "sec_active"      => 1,
    "sec_old_cat_id"  => {$sec_id}
);
...
EOT;
                ...
                if ($o_items !== false) {
                    ...
                    foreach ($a_items as $a_item) {
                        ...
                        if (file_exists($doc_root . $item_file_name) === false) {
...
EOT;
                            ...
                            if ($o_item_data !== false) {
                                ...
                                foreach ($a_item_data as $a_data) {
                                    $field_id = $a_data['fieldid'];
                                    $field_data = $a_data['data_txt'];
                                    if (trim($field_data) != '') {
...
EOT;
    ...
    fwrite($o_section_files, ");\n");
    fclose($o_section_files);
}
$o_sections->free();
?>
